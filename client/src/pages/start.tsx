...
const Start: NextPage = () => {
  ...
  const createSignature = async () => {
    ...
    const message = new TextEncoder().encode(
      `Signing this message for verification: ${+new Date()}`
    );
...
>>>>>>> Stashed changes
          ...
          <Text color="#4B4B67" fontSize="2rem">
            GitHub connection is required to check your profile and
            contributions from our list
          </Text>
          ...
          <Flex justify="space-between" align="center" w="100%">
            <WalletMultiButton
              ...
              style={{
                height: '4.5rem',
                opacity:
                  !session || !session.user ? '0.6' : publicKey ? '0.5' : '1',
                width: '25rem',
                fontWeight: 600,
                borderRadius: '1.5rem',
                alignSelf: 'start',
                fontSize: '2.2rem',
                background:
                  'linear-gradient(93.65deg, #2546BB 3.63%, #2E22B9 98.31%)',
              }}
            >
              {publicKey ? null : (
                <Icon as={FaWallet} transform="translate(-0.5rem, 0)" />
              )}
              {publicKey
                ? truncatedPublicKey(publicKey.toBase58())
                : 'Connect Wallet'}
            </WalletMultiButton>

            <Box>{publicKey ? <Icon as={FaCheck} color="green" /> : null}</Box>
          </Flex>
>>>>>>> Stashed changes
          ...
          <Text color="#4B4B67" fontSize="2rem">
            We need to verify a message signature to make sure that the public
            key is correct
          </Text>
